...
class MirrorStatus:
    ...
    STATUS_CLONING = "Salin...♻️"
    STATUS_WAITING = "Antri...💤"
    STATUS_PAUSE = "Berhenti...⛔️"
    STATUS_ARCHIVING = "Arsip...🔐"
    STATUS_EXTRACTING = "Ekstrak...📂"
    STATUS_SPLITTING = "Bagi...✂️"
    STATUS_CHECKING = "Dicek...📝"
...
def get_readable_message():
    with download_dict_lock:
        ...
        for index, download in enumerate(list(download_dict.values())[COUNT:], start=1):
            ...
            if download.status() not in [MirrorStatus.STATUS_SPLITTING, MirrorStatus.STATUS_SEEDING]:
                ...
                if download.status() in [MirrorStatus.STATUS_DOWNLOADING,
                                         MirrorStatus.STATUS_WAITING,
                                         MirrorStatus.STATUS_PAUSE]:
                    msg += f"\n<b>Didownload:</b> {get_readable_file_size(download.processed_bytes())} dari {download.size()}"
                elif download.status() == MirrorStatus.STATUS_UPLOADING:
                    msg += f"\n<b>Diupload:</b> {get_readable_file_size(download.processed_bytes())} dari {download.size()}"
                elif download.status() == MirrorStatus.STATUS_CLONING:
                    msg += f"\n<b>Disalin:</b> {get_readable_file_size(download.processed_bytes())} dari {download.size()}"
                elif download.status() == MirrorStatus.STATUS_ARCHIVING:
                    msg += f"\n<b>Diarsip:</b> {get_readable_file_size(download.processed_bytes())} dari {download.size()}"
                elif download.status() == MirrorStatus.STATUS_EXTRACTING:
                    msg += f"\n<b>Diekstrak:</b> {get_readable_file_size(download.processed_bytes())} dari {download.size()}"
                msg += f"\n<b>Speed:</b> {download.speed()} | <b>Est:</b> {download.eta()}"
                try:
                ...
                msg += f" | <b>Diupload: </b>{get_readable_file_size(download.torrent_info().uploaded)}"
        ...
        if STATUS_LIMIT is not None and tasks > STATUS_LIMIT:
            msg += f"<b>Hal:</b> {PAGE_NO}/{pages} | <b>Tugas:</b> {tasks}\n"
            buttons = ButtonMaker()
            buttons.sbutton("Sebelum", "status pre")
            buttons.sbutton("Sesudah", "status nex")
