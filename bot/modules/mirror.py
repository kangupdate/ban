...
from bot.helper.ext_utils.bot_utils import is_url, is_magnet, is_mega_link, is_gdrive_link, get_content_type, get_readable_message, get_readable_time, mulai
...
def _mirror(bot, message, isZip=False, extract=False, isQbit=False, isLeech=False, pswd=None, multi=0, qbsd=False):
    ...
    if not is_url(link) and not is_magnet(link) and not ospath.exists(link):
        ...
        help_msg += "\n\n<b>Multi tautan hanya dengan membalas tautan atau file pertama:</b>"
    ...
    if is_gdrive_link(link):
        if not isZip and not extract and not isLeech:
            gmsg = f"Use /{BotCommands.CloneCommand} untuk menyalin Google Drive file/folder\n\n"
            gmsg += f"Use /{BotCommands.ZipMirrorCommand} utk membuat zip ke Google Drive folder\n\n"
            gmsg += f"Use /{BotCommands.UnzipMirrorCommand} utk ekstrak Google Drive file arsip"
