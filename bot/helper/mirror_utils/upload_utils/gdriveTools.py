    ...
    def drive_list(self, fileName, stopDup=False, noMulti=False, isRecursive=True, itemType=""):
        ...
        for index, parent_id in enumerate(DRIVES_IDS):
            ...
            if not response["files"] and noMulti:
                ...
                continue
            if not Title:
                msg += f'<h4>Hasil Pencarian Untuk {fileName}</h4>'
                Title = True
        ...
        msg = f"<b>Found {contents_count} hasil untuk <i>{fileName}</i></b>"
        buttons = ButtonMaker()
    ...
    def count(self, link):
        ...
        try:
            ...
            if mime_type == self.__G_DRIVE_DIR_MIME_TYPE:
                ...
                msg += f'\n\n<b> ↳Ukuran: </b>{get_readable_file_size(self.total_bytes)}'
                msg += f'\n<b> ↳Type: </b>Folder'
