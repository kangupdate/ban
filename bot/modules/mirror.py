...
def _mirror(bot, message, isZip=False, extract=False, isQbit=False, isLeech=False, pswd=None, multi=0, qbsd=False):
    ...
    if is_gdrive_link(link):
        if not isZip and not extract and not isLeech:
            gmsg = f"Use /{BotCommands.CloneCommand} untuk mengkloning file/folder Google Drive\n\n"
            gmsg += f"Use /{BotCommands.ZipMirrorCommand} untuk membuat zip folder Google Drive\n\n"
            gmsg += f"Use /{BotCommands.UnzipMirrorCommand} untuk mengekstrak file arsip Google Drive"
